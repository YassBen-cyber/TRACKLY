...
export function SetPasswordCard() {
  ...
  if (success) {
    return (
      <div className="bg-green-500/10 border border-green-500/20 p-6 rounded-3xl flex items-center gap-4 text-green-400 shadow-sm glass-panel">
        ...
        <div>
          <h3 className="font-semibold text-lg text-zinc-900">Mot de passe défini avec succès !</h3>
  ...
  return (
    <div className="glass-panel p-6 sm:p-8 rounded-3xl border border-zinc-200 shadow-2xl relative overflow-hidden">
      ...
      <div className="relative z-10">
        <div className="flex justify-between items-start mb-2">
          <h3 className="text-xl font-bold tracking-tight text-zinc-900">Sécuriser mon compte</h3>
          <Button variant="ghost" size="icon" onClick={handleDismiss} className="text-zinc-600 hover:text-zinc-900 rounded-full">
            <X className="h-5 w-5" />
          </Button>
        </div>
        ...
        <form onSubmit={onSubmit} className="space-y-4 max-w-md">
          <div className="space-y-2 group">
            <Label htmlFor="new-password" className="text-zinc-700 transition-colors group-focus-within:text-primary">Nouveau mot de passe</Label>
            <Input 
              id="new-password" 
              type="password" 
              value={password}
          ...
          <div className="flex items-center gap-3">
            <Button type="submit" disabled={isLoading} className="h-11 px-6 rounded-xl bg-primary hover:bg-primary/90 text-white shadow-lg shadow-primary/20 transition-all hover:scale-105">
              {isLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Lock className="mr-2 h-4 w-4" />}
              Enregistrer mon mot de passe
            </Button>
            <Button type="button" variant="ghost" onClick={handleDismiss} className="h-11 px-4 rounded-xl text-zinc-600 hover:text-zinc-900">
              Plus tard
            </Button>
          </div>
        </form>
