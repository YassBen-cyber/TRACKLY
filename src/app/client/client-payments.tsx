    ...
    <div className="glass-panel p-6 sm:p-8 rounded-3xl mt-8">
      ...
      {pendingPayments.length > 0 && (
        <div className="mb-8 space-y-4">
          <h4 className="text-sm font-semibold text-zinc-600 uppercase tracking-wider">En attente de paiement</h4>
          {pendingPayments.map(payment => (
            <div key={payment.id} className="p-5 rounded-2xl border border-emerald-500/30 bg-emerald-500/5 relative overflow-hidden group">
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 relative z-10">
                ...
                <div className="flex items-center gap-4 w-full sm:w-auto bg-white p-2 pl-4 rounded-xl border border-zinc-200">
                  <span className="text-2xl font-black text-zinc-900">{payment.amount} €</span>
                  <Button 
      ...
      {pastPayments.length > 0 && (
        <div className="space-y-4">
          <h4 className="text-sm font-semibold text-zinc-600 uppercase tracking-wider">Historique</h4>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {pastPayments.map(payment => (
              <div key={payment.id} className="p-4 rounded-xl border border-zinc-200 bg-white flex justify-between items-center">
                <div>
                  <div className="font-bold text-zinc-700">{payment.title}</div>
                ...
                <div className="text-right">
                  <div className="font-mono font-bold text-zinc-600">{payment.amount} €</div>
                  {payment.status === 'paid' && (
