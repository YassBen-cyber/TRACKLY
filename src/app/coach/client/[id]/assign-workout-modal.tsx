        ...
        <form onSubmit={onSubmit} className="flex flex-col h-full">
            ...
            {templates.length === 0 ? (
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  <label className="text-sm text-zinc-700">1. Choisir la séance</label>
                  <select 
                    value={selectedTemplateId} 
                    onChange={e => setSelectedTemplateId(e.target.value)}
                    className="flex h-11 w-full items-center justify-between rounded-xl border border-zinc-300 bg-white px-3 py-2 text-sm text-zinc-900 focus:outline-none focus:ring-2 focus:ring-blue-500/50 appearance-none"
                  >
                    <option value="" disabled className="bg-zinc-50 text-zinc-600">Sélectionner...</option>
                    {templates.map(t => (
                      <option key={t.id} value={t.id} className="bg-zinc-50 text-zinc-900 py-2">
                ...
                <div className="space-y-2">
                  <label className="text-sm text-zinc-700">2. Choisir une date (selon ses disponibilités)</label>
                  ...
                  <select 
                    ...
                    className="flex h-11 w-full items-center justify-between rounded-xl border border-zinc-300 bg-white px-3 py-2 text-sm text-zinc-900 focus:outline-none focus:ring-2 focus:ring-blue-500/50 appearance-none"
                  >
                    <option value="" disabled className="bg-zinc-50 text-zinc-600">Sélectionner une date...</option>
                    {sortedDates.map(date => {
                      ...
                      return (
                        <option key={date} value={date} className="bg-zinc-50 text-zinc-900 py-2">
                          {dateStr} (Dispo: {slotsStr})
                        </option>
                      )
                    })}
                    <option value="custom" className="bg-zinc-50 text-blue-400 font-bold py-2">
                ...
                {selectedDate === 'custom' && (
                  <div className="space-y-2 pt-2 animate-in fade-in zoom-in-95">
                    <label className="text-sm text-zinc-700">Date personnalisée</label>
                    <input 
                      type="date" 
                      required 
                      min={new Date().toISOString().split('T')[0]}
                      onChange={(e) => setSelectedDate(e.target.value)}
                      className="flex h-11 w-full rounded-xl border border-zinc-300 bg-white px-3 py-2 text-sm text-zinc-900 focus:outline-none focus:ring-2 focus:ring-blue-500 [color-scheme:dark]"
                    />
                  </div>
                )}
              </div>
          ...
          <div className="p-6 border-t border-zinc-200 bg-white mt-auto">
            <DialogFooter>
              ...
              <Button type="submit" disabled={isLoading || templates.length === 0} className="rounded-xl bg-blue-600 hover:bg-blue-700 text-zinc-900 shadow-lg shadow-blue-600/20">
