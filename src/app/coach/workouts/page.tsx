      ...
      {(!templates || templates.length === 0) ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {templates.map(template => (
            <div key={template.id} className="glass-panel p-6 rounded-3xl flex flex-col group hover:border-primary/30 transition-all border border-zinc-200">
              ...
              {template.description && (
                <p className="text-sm text-zinc-600 mb-4 line-clamp-2">{template.description}</p>
              )}
              
              <div className="mt-auto space-y-3 pt-4 border-t border-zinc-200">
